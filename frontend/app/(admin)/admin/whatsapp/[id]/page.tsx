  ...
  return (
    <div className="space-y-1.5">
      <div className="flex h-2 w-full overflow-hidden rounded-full bg-[#F0EBE3]">
        <div className="h-full bg-[#A68B67] transition-all" style={{ width: `${sentPct}%` }} />
      ...
      <div className="flex gap-4 text-[11.5px] font-public-sans text-[#9CA3AF]">
        <span className="flex items-center gap-1.5">
          <span className="inline-block w-2 h-2 rounded-full bg-[#A68B67]" />
          {sent} sent
        </span>
        <span className="flex items-center gap-1.5">
...
function StatCard({ label, value, sub }: { label: string; value: string | number; sub?: string }) {
  return (
    <div className="bg-white border border-[#E5E1D8] rounded-lg px-5 py-4">
      <p className="text-[11px] font-[700] font-public-sans text-[#9CA3AF] uppercase tracking-[0.07em] mb-1.5">{label}</p>
      <p className="text-[24px] font-playfair font-[600] text-[#1A1A1A] leading-none tabular-nums">{value}</p>
...
export default function BroadcastDetailPage({ params }: { params: Promise<{ id: string }> }) {
      ...
      <div className="p-8 max-w-5xl space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="bg-white border border-[#E5E1D8] rounded-lg px-5 py-4">
        ...
        <div className="bg-white border border-[#E5E1D8] rounded-lg overflow-hidden">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="px-5 py-3.5 border-b border-[#F5F0E8] flex gap-6">
  ...
  if (!broadcast) {
    return (
      <div className="p-8 max-w-5xl">
        <Link href="/admin/whatsapp" className="flex items-center gap-1.5 text-[13px] font-public-sans text-[#9CA3AF] hover:text-[#444748] mb-6">
  ...
  return (
    <div className="p-8 max-w-5xl space-y-6">
      ...
      {/* Back */}
      <Link href="/admin/whatsapp" className="inline-flex items-center gap-1.5 text-[13px] font-public-sans text-[#9CA3AF] hover:text-[#444748] transition-colors">
        <ArrowLeft size={14} /> Back to broadcasts
      </Link>
      ...
      <div className="flex items-start justify-between">
        <div className="flex items-start gap-3">
          <div className="mt-0.5 w-9 h-9 rounded-lg bg-[#F5F0E8] flex items-center justify-center shrink-0">
          ...
          <div>
            ...
            <p className="text-[13px] font-public-sans text-[#9CA3AF] mt-0.5">
              Template: <code className="text-[12px] bg-[#F5F0E8] text-[#A68B67] px-1.5 py-0.5 rounded">{broadcast.templateName}</code>
      ...
      {/* Progress */}
      <div className="bg-white border border-[#E5E1D8] rounded-lg px-6 py-5">
      ...
      {/* Recipients table */}
      <div className="bg-white border border-[#E5E1D8] rounded-lg overflow-hidden">
        <div className="px-5 py-3.5 border-b border-[#E5E1D8] bg-[#FCFAFA] flex items-center justify-between">
          <h2 className="text-[13px] font-[600] font-public-sans text-[#1A1A1A]">Recipients</h2>
            ...
            <tbody>
              {!broadcast.recipients?.length ? (
                ...
                broadcast.recipients.map((r) => (
                  <tr key={r.id} className="border-b border-[#F5F0E8] hover:bg-[#FCFAFA] transition-colors">
                    <td className="pl-5 pr-2 py-3">
                      {RECIPIENT_STATUS_ICON[r.status] ?? RECIPIENT_STATUS_ICON.PENDING}
                    </td>
                    <td className="px-2 py-3">
                      <span className="text-[13px] font-mono text-[#444748]">{r.phone}</span>
                    </td>
                    <td className="px-2 py-3">
                      <span className="text-[13px] font-public-sans text-[#444748]">{r.name ?? '—'}</span>
                    </td>
                    <td className="px-2 py-3">
                      <span className={cn(
                        'inline-flex items-center px-2 py-0.5 rounded text-[11px] font-[600] font-public-sans',
                        r.status === 'SENT'    ? 'bg-[#6B7E6B]/10 text-[#6B7E6B]' :
