...
export default function AdminPage() {
  ...
  const statCards = [
    { label: "Approved brands", value: s?.totalBrands ?? 0,                                                           icon: Store,      color: "text-[#C8956C]", bg: "bg-[#F5EDE6]" },
  ...
  return (
    <div className="max-w-5xl space-y-8">
      <div>
        ...
        <p className="mt-1 text-sm text-[#6B6056]">Platform overview and brand management.</p>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {statCards.map((s) => (
          <div key={s.label} className="bg-white rounded-xl border border-[#E8E0D8] p-4 shadow-warm">
      ...
      {/* Pending approvals */}
      <div className="bg-white rounded-xl border border-[#E8E0D8] shadow-warm">
        <div className="flex items-center justify-between p-5 border-b border-[#E8E0D8]">
          <div className="flex items-center gap-2">
            ...
            {pending.length > 0 && (
              <span className="h-5 min-w-5 px-1.5 rounded-full bg-[#C8956C] text-white text-[10px] font-semibold flex items-center justify-center">
                {pending.length}
              </span>
            )}
          </div>
        ...
        {pending.length === 0 ? (
          <div className="py-12 text-center">
            ...
            <p className="text-sm text-[#6B6056]">All caught up! No pending applications.</p>
          </div>
        ) : (
          <div className="divide-y divide-[#E8E0D8]">
            {pending.map((brand: any) => (
              <div key={brand.id} className="flex items-center gap-4 px-5 py-4">
                {/* Logo placeholder */}
                <div className="h-10 w-10 rounded-full bg-[#F5EDE6] flex items-center justify-center shrink-0 font-heading font-bold text-[#C8956C]">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-0.5">
                    <p className="text-xs text-[#6B6056]">{brand.user?.email}</p>
                    {brand.category?.length > 0 && (
                      <span className="text-xs text-[#6B6056]">{brand.category.join(", ")}</span>
                    )}
                  ...
                  {brand.instagramHandle && (
                    <p className="text-xs text-[#C8956C] mt-0.5">@{brand.instagramHandle}</p>
                  )}
                </div>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <button
                    ...
                    className="flex items-center gap-1.5 h-8 px-3 rounded-lg border border-[#E8E0D8] text-xs font-medium text-[#C0392B] hover:bg-[#FEF2F2] hover:border-[#C0392B] transition-colors disabled:opacity-50"
                  >
      ...
      <div className="bg-white rounded-xl border border-[#E8E0D8] shadow-warm">
        ...
        {promotions.length === 0 ? (
          <div className="py-10 text-center">
            <Zap className="h-8 w-8 text-[#E8E0D8] mx-auto mb-2" />
            <p className="text-sm text-[#6B6056]">No pending promotions to review.</p>
          </div>
        ) : (
          <div className="divide-y divide-[#E8E0D8]">
            {promotions.map((promo: any) => (
              <div key={promo.id} className="flex items-center gap-4 px-5 py-4">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-[#6B6056] mt-0.5">{promo.brandProfile?.brandName}</p>
                  <div className="flex items-center gap-3 mt-1 text-xs text-[#6B6056]">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          ...
          <div key={href} className="flex items-center gap-3 p-4 bg-white rounded-xl border border-[#E8E0D8] hover:border-[#C8956C] hover:shadow-warm-md transition-all cursor-pointer group" onClick={() => {}}>
            <div className="flex-1">
              <p className="text-sm font-semibold text-[#1A1A1A] group-hover:text-[#C8956C] transition-colors">{label}</p>
              <p className="text-xs text-[#6B6056] mt-0.5">{desc}</p>
            </div>
            <ChevronRight className="h-4 w-4 text-[#E8E0D8] group-hover:text-[#C8956C] transition-colors" />
          </div>
        ))}
      </div>
    </div>
