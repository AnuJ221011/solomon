    ...
    <div className="max-w-4xl space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-[#444748] mt-0.5">Orders via your share link pay 0% commission.</p>
        </div>
      ...
      {links && links.length > 0 && (
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            <div key={s.label} className="bg-white rounded-lg border border-[#E5E1D8] p-4 shadow-warm text-center">
      ...
      {/* Create form */}
      {showCreate && (
        <div className="bg-white rounded-lg border border-[#E5E1D8] shadow-warm p-5 space-y-4">
          ...
          <div className="flex gap-3">
            <button onClick={() => setShowCreate(false)} className="flex-1 h-9 rounded-lg border border-[#E5E1D8] text-sm font-medium text-[#444748] hover:bg-[#F9F7F2]">Cancel</button>
            <button onClick={() => createMutation.mutate()} disabled={createMutation.isPending}
              className="flex-1 h-9 rounded-lg bg-[#1A1A1A] hover:bg-[#8B7055] text-white text-sm font-medium flex items-center justify-center gap-2 disabled:opacity-60">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20 bg-white rounded-lg border border-[#E5E1D8]">
          <Link2 className="h-10 w-10 text-[#E5E1D8] mx-auto mb-3" />
          <p className="font-heading text-lg text-[#1A1A1A] mb-1">No share links yet</p>
          <p className="text-sm text-[#444748]">Create a link and send it to your wholesale customers — 0% commission on all orders.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {links.map((link: any) => (
            <div key={link.id} className="bg-white rounded-lg border border-[#E5E1D8] shadow-warm p-5">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <p className="font-medium text-[#1A1A1A]">{link.slug ?? link.token.slice(0, 16) + "…"}</p>
                    <span className="text-xs text-[#444748] bg-[#F5F0E8] px-2 py-0.5 rounded-full">{link.target}</span>
                    {!link.isActive && <span className="text-xs text-[#C0392B] bg-[#FEF2F2] px-2 py-0.5 rounded-full">Inactive</span>}
                  </div>
                  {link.customMessage && <p className="text-xs text-[#444748] mb-2">"{link.customMessage}"</p>}
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <button onClick={() => copyLink(link.token, link.slug)} className="h-8 w-8 rounded-lg border border-[#E5E1D8] flex items-center justify-center text-[#444748] hover:border-[#A68B67] hover:text-[#A68B67] transition-colors" title="Copy link">
                  ...
                  <a href={`https://wa.me/?text=${encodeURIComponent(`${window.location.origin}/share-links/${link.slug ?? link.token}`)}`} target="_blank" rel="noopener noreferrer"
                    className="h-8 w-8 rounded-lg border border-[#E5E1D8] flex items-center justify-center text-[#444748] hover:border-[#25D366] hover:text-[#25D366] transition-colors" title="Share on WhatsApp">
                    <ExternalLink className="h-4 w-4" />
                  </a>
                  <button onClick={() => toggleMutation.mutate({ id: link.id, isActive: !link.isActive })}
                    className="text-[#444748] hover:text-[#A68B67]" title={link.isActive ? "Deactivate" : "Activate"}>
                  ...
                  <button onClick={() => { if (confirm("Delete this link?")) deleteMutation.mutate(link.id); }}
                    className="text-[#444748] hover:text-[#C0392B]"><Trash2 className="h-4 w-4" /></button>
                </div>
              </div>
            </div>
          ))}
...
const inp = "w-full h-10 px-3 rounded-lg border border-[#E5E1D8] bg-[#F9F7F2] text-sm focus:outline-none focus:border-[#A68B67] focus:bg-white transition-colors";