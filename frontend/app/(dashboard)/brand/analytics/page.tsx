...
export default function BrandAnalyticsPage() {
  ...
  if (isLoading) {
    return <div className="flex justify-center py-20"><Loader2 className="h-6 w-6 animate-spin text-[#C8956C]" /></div>;
  }
  ...
  return (
    <div className="max-w-5xl space-y-8">
      <div>
        ...
        <p className="mt-1 text-sm text-[#6B6056]">Performance overview for your brand.</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { label: "Product views",     value: totalViews,    icon: Eye,         color: "text-[#C8956C]", bg: "bg-[#F5EDE6]" },
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border border-[#E8E0D8] shadow-warm">
          ...
          {products.length === 0 ? (
            <div className="py-10 text-center text-sm text-[#6B6056]">No products yet.</div>
          ) : (
                ...
                return (
                  <div key={p.id} className="flex items-center gap-4 px-5 py-3">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-[#6B6056] mt-0.5">{p.viewCount ?? 0} views · {rate}% conversion</p>
                    </div>
        ...
        <div className="bg-white rounded-xl border border-[#E8E0D8] shadow-warm">
          ...
          {shareLinks.length === 0 ? (
            <div className="py-10 text-center text-sm text-[#6B6056]">No share links yet.</div>
          ) : (
            <div className="divide-y divide-[#E8E0D8]">
              {shareLinks.slice(0, 6).map((link: any) => (
                <div key={link.id} className="flex items-center gap-4 px-5 py-3">
                  ...
                  <div className="text-right shrink-0">
                    ...
                    <p className="text-xs text-[#6B6056]">₹{Number(link.revenueInr ?? 0).toLocaleString("en-IN")} rev</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          <div key={s.label} className="bg-[#FAFAF8] rounded-xl border border-[#E8E0D8] p-4">
            <p className="text-2xl font-bold text-[#1A1A1A]">{s.value}</p>
            <p className="text-xs font-medium text-[#1A1A1A] mt-1">{s.label}</p>
            <p className="text-xs text-[#6B6056]">{s.sub}</p>
          </div>
        ))}
      </div>
    </div>
