...
export default function BrandDashboardPage() {
  ...
  const stats = [
    ...
    { label: "Orders this month", value: d?.ordersThisMonth ?? 0,                                                                icon: ShoppingBag,  color: "text-[#C8956C]", bg: "bg-[#F5EDE6]" },
    ...
    <div className="max-w-6xl space-y-8">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-[#6B6056]">Here's what's happening with your brand today.</p>
        </div>
        <Link
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        {p && (
          <div className="lg:col-span-2 bg-white rounded-xl border border-[#E8E0D8] p-6 shadow-warm">
                ...
                {Object.entries(p.nextLevel.criteria ?? {}).map(([key, target]: any) => {
                  ...
                  return (
                    <div key={key}>
                      ...
                      <div className="h-1.5 bg-[#F5EDE6] rounded-full overflow-hidden">
                        <div
        ...
        <div className="bg-white rounded-xl border border-[#E8E0D8] p-6 shadow-warm">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/brand/share-links" className="text-xs text-[#C8956C] hover:text-[#B07D57] flex items-center gap-1">
          ...
          {d?.shareLinks?.length > 0 ? (
            <div className="space-y-3">
              {d.shareLinks.slice(0, 3).map((link: any) => (
                <div key={link.id} className="flex items-center justify-between py-2 border-b border-[#E8E0D8] last:border-0">
                  <div>
                    <p className="text-sm font-medium text-[#1A1A1A] truncate max-w-[140px]">
                      {link.slug ?? link.token.slice(0, 8) + "…"}
                    </p>
                    <p className="text-xs text-[#6B6056]">{link.viewCount} views · {link.signupCount} signups</p>
                  </div>
            ...
            <div className="text-center py-6">
              <Link2 className="h-8 w-8 text-[#E8E0D8] mx-auto mb-2" />
              <p className="text-sm text-[#6B6056]">No share links yet</p>
              <Link href="/brand/share-links" className="mt-2 text-xs text-[#C8956C] font-medium">Create one →</Link>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          <Link
            key={href}
            href={href}
            className="flex items-center gap-3 p-4 bg-white rounded-xl border border-[#E8E0D8] hover:border-[#C8956C] hover:shadow-warm-md transition-all group"
          >
            <div className="h-9 w-9 rounded-lg bg-[#F5EDE6] flex items-center justify-center shrink-0">
              <Icon className="h-5 w-5 text-[#C8956C]" />
            </div>
            <div>
              <p className="text-sm font-medium text-[#1A1A1A] group-hover:text-[#C8956C] transition-colors">{label}</p>
              {badge && <span className="text-[10px] text-[#2D6A4F] font-semibold">{badge}</span>}
            </div>
            <ChevronRight className="h-4 w-4 text-[#E8E0D8] ml-auto group-hover:text-[#C8956C] transition-colors" />
          </Link>
        ))}
      </div>
    </div>
...
function DashboardSkeleton() {
  return (
    <div className="max-w-6xl space-y-8">
      <div className="h-8 w-64 bg-[#F5EDE6] rounded animate-pulse" />
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[1,2,3,4].map((i) => (
          <div key={i} className="bg-white rounded-xl border border-[#E8E0D8] p-4 h-28 animate-pulse" />
        ))}
      </div>
