    ...
    <div className="max-w-5xl space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20 bg-white rounded-lg border border-[#E5E1D8]">
          <p className="text-[#444748]">No orders found.</p>
        </div>
      ) : (
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-[#E5E1D8]">
                {orders.map((order: any) => (
                  <tr key={order.id} className="hover:bg-[#F9F7F2] transition-colors">
                    <td className="px-4 py-3 font-mono text-xs text-[#444748]">{order.id.slice(0, 10)}…</td>
                    <td className="px-4 py-3">
                      <p className="font-medium text-[#1A1A1A]">{order.buyer?.buyerProfile?.businessName ?? order.buyer?.name}</p>
                      <p className="text-xs text-[#444748]">{order.buyer?.buyerProfile?.countryCode}</p>
                    </td>
                    <td className="px-4 py-3 text-[#444748]">{order.items?.length ?? 0}</td>
                    <td className="px-4 py-3 font-medium">₹{Number(order.totalInr).toLocaleString("en-IN")}</td>
                    <td className="px-4 py-3">
                      <span className={cn("px-2 py-0.5 rounded-full text-xs font-semibold", STATUS_STYLE[order.status] ?? "bg-[#F9F7F2] text-[#444748]")}>
                        {STATUS_LABELS[order.status] ?? order.status}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-[#444748] text-xs">{new Date(order.createdAt).toLocaleDateString()}</td>
                    <td className="px-4 py-3">
                      <div className="flex gap-2">
                        ...
                        {(order.status === "CONFIRMED" || order.status === "PROCESSING") && (
                          <button onClick={() => setDispatchModal({ orderId: order.id })}
                            className="h-7 px-2.5 rounded-lg bg-[#1A1A1A] text-white text-xs font-medium hover:bg-[#8B7055] transition-colors flex items-center gap-1">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div className="relative bg-white rounded-lg border border-[#E5E1D8] shadow-warm-lg p-6 w-full max-w-sm space-y-4">
            ...
            <div className="flex gap-3">
              <button onClick={() => setDispatchModal(null)} className="flex-1 h-10 rounded-lg border border-[#E5E1D8] text-sm font-medium hover:bg-[#F9F7F2]">Cancel</button>
              <button onClick={() => dispatchMutation.mutate(dispatchModal)} disabled={dispatchMutation.isPending}
                className="flex-1 h-10 rounded-lg bg-[#1A1A1A] text-white text-sm font-medium hover:bg-[#8B7055] flex items-center justify-center gap-2 disabled:opacity-60">
