  ...
  return (
    <div className="max-w-5xl space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: "Total orders",   value: orders.length,                          icon: ShoppingBag, color: "text-[#C8956C]", bg: "bg-[#F5EDE6]" },
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        {/* Recent orders */}
        <div className="lg:col-span-2 bg-white rounded-xl border border-[#E8E0D8] shadow-warm">
          <div className="flex items-center justify-between p-5 border-b border-[#E8E0D8]">
            ...
            <Link href="/buyer/orders" className="text-xs text-[#C8956C] hover:text-[#B07D57] flex items-center gap-1">
          ...
          {orders.length > 0 ? (
            <div className="divide-y divide-[#E8E0D8]">
              {orders.slice(0, 5).map((order: any) => {
                ...
                return (
                  ...
                  >
                    <div className="h-10 w-10 rounded-lg bg-[#F5EDE6] flex items-center justify-center shrink-0">
                      <Package className="h-5 w-5 text-[#C8956C]" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-[#1A1A1A] truncate group-hover:text-[#C8956C] transition-colors">
            ...
            <div className="py-12 text-center">
              <ShoppingBag className="h-8 w-8 text-[#E8E0D8] mx-auto mb-2" />
              <p className="text-sm text-[#6B6056]">No orders yet</p>
              <Link href="/shop" className="mt-2 text-xs text-[#C8956C] font-medium">Start browsing →</Link>
            </div>
          )}
        </div>
        ...
        <div className="space-y-4">
          {/* Wallet */}
          <div className="bg-white rounded-xl border border-[#E8E0D8] p-5 shadow-warm">
            <div className="flex items-center justify-between mb-3">
              <h2 className="font-heading text-sm font-semibold text-[#1A1A1A]">Wallet</h2>
              <Link href="/buyer/wallet" className="text-xs text-[#C8956C]">Details</Link>
            </div>
            <p className="text-2xl font-bold text-[#1A1A1A]">₹{Number(walletBalance).toFixed(0)}</p>
            <p className="text-xs text-[#6B6056] mt-0.5">Available store credit</p>
            {activeCredits.length > 0 && (
              <div className="mt-3 pt-3 border-t border-[#E8E0D8] space-y-1.5">
                {activeCredits.slice(0, 2).map((c: any) => (
                  <div key={c.id} className="flex justify-between text-xs">
                    <span className="text-[#6B6056] truncate">{c.reason}</span>
          ...
          <div className="bg-white rounded-xl border border-[#E8E0D8] p-5 shadow-warm">
            <div className="flex items-center justify-between mb-3">
              ...
              <Link href="/buyer/referrals" className="text-xs text-[#C8956C]">Details</Link>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="p-2.5 rounded-lg bg-[#F5EDE6]">
                <p className="text-lg font-bold text-[#1A1A1A]">{referralStats.totalReferrals ?? 0}</p>
                <p className="text-[10px] text-[#6B6056]">brands referred</p>
              </div>
              <div className="p-2.5 rounded-lg bg-[#E8F5EE]">
                <p className="text-lg font-bold text-[#1A1A1A]">{referralStats.rewardsEarned ?? 0}</p>
                <p className="text-[10px] text-[#6B6056]">rewards earned</p>
              </div>
            </div>
            <Link href="/buyer/referrals" className="mt-3 block text-xs text-[#C8956C] font-medium">
