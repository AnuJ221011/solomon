    ...
    <div className="max-w-5xl space-y-8">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20">
          <Heart className="h-10 w-10 text-[#E5E1D8] mx-auto mb-3" />
          <p className="font-heading text-lg text-[#1A1A1A] mb-1">Nothing saved yet</p>
          <p className="text-sm text-[#444748] mb-4">Save products and brands to find them quickly.</p>
          <Link href="/shop" className="text-sm text-[#A68B67] font-medium">Browse products →</Link>
        </div>
      ) : (
        <>
          {products.length > 0 && (
            ...
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-4">
                {brands.map((brand: any) => (
                  <div key={brand.id} className="relative group bg-white rounded-lg border border-[#E5E1D8] p-4 flex flex-col items-center text-center hover:border-[#A68B67] transition-all">
                    <button onClick={() => unsaveBrandMutation.mutate(brand.id)}
                      className="absolute top-2 right-2 h-6 w-6 rounded-full bg-white border border-[#E5E1D8] flex items-center justify-center text-[#444748] hover:text-[#C0392B] opacity-0 group-hover:opacity-100 transition-all">
                      <X className="h-3 w-3" />
                    </button>
                    <div className="h-12 w-12 rounded bg-[#F5F0E8] overflow-hidden flex items-center justify-center mb-2">
                      {brand.logoUrl ? (
                        <Image src={brand.logoUrl} alt={brand.brandName} width={48} height={48} className="object-cover" />
                      ) : (
                        <span className="font-heading text-xl font-bold text-[#A68B67]">{brand.brandName?.charAt(0)}</span>
                      )}
                    </div>
                    <Link href={`/brands/${brand.slug}`} className="text-xs font-semibold text-[#1A1A1A] hover:text-[#A68B67] transition-colors line-clamp-1">
                      {brand.brandName}
                    </Link>
                  </div>
                ))}
