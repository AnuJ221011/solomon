...
import { ArrowLeft, Upload, X, Plus, Loader2, Trash2, RefreshCw, Check, Search, Sparkles, RotateCcw, Video, AlertCircle } from 'lucide-react'
...
export default function NewProductPage() {
  ...
  const { score, maxScore, rules } = useMemo(
    () => calcListingScore(form, priceTiers, media, attrs, craft),
    [form, priceTiers, media, attrs, craft],
  )
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const sortedTiers = [...validTiers].sort((a, b) => Number(a.moq) - Number(b.moq))
    ...
    try {
      ...
      const res = await api.post('/products', {
        ...
        wholesalePriceInr: Number(sortedTiers[0].priceInr),
        moq:               Number(sortedTiers[0].moq),
        ...
        priceTiers:        sortedTiers.map((t) => ({ moq: Number(t.moq), priceInr: Number(t.priceInr) })),
        // Attributes
      ...
      if (variantCombos.length > 0 && productId) {
        ...
        await api.post(`/products/${productId}/variants/bulk`, {
          variants: variantCombos.map((combo) => {
            ...
            const sortedVpTiers = [...vp.tiers].sort((a, b) => Number(a.moq) - Number(b.moq))
            return {
              sku:        autoSku,
