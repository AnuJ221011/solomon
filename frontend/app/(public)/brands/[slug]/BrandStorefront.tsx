...
export function BrandStorefront({ slug }: Props) {
  ...
  if (isError || !brand) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-center">
        ...
        <Link href="/brands" className="text-sm text-[#C8956C] font-medium">← All brands</Link>
      </div>
    );
  }
  ...
  return (
    <div>
      {/* Banner */}
      <div className="relative h-48 sm:h-64 lg:h-80 bg-[#F5EDE6] overflow-hidden">
        {brand.bannerUrl ? (
          ...
          <div
            ...
            style={{
              background: "linear-gradient(135deg, #F5EDE6 0%, #E8D5C4 50%, #D4B8A0 100%)",
            }}
          />
        )}
        {/* Back */}
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex items-end gap-5 -mt-10 mb-6 relative">
          {/* Logo */}
          <div className="h-20 w-20 rounded-2xl border-4 border-white bg-[#F5EDE6] overflow-hidden shrink-0 shadow-warm-md">
            {brand.logoUrl ? (
              <Image src={brand.logoUrl} alt={brand.brandName} width={80} height={80} className="object-cover" />
            ) : (
              <div className="h-full w-full flex items-center justify-center">
                <span className="font-heading text-3xl font-bold text-[#C8956C]">
          ...
          <div className="pb-1">
            ...
            <div className="flex items-center gap-3 mt-1 flex-wrap">
              {brand.category.map((c: string) => (
                <span key={c} className="text-sm text-[#6B6056]">{c}</span>
              ))}
            </div>
          </div>
        </div>
        ...
        {/* Stats bar */}
        <div className="flex flex-wrap gap-6 mb-8 pb-6 border-b border-[#E8E0D8]">
        ...
        {/* Tabs */}
        <div className="flex gap-6 border-b border-[#E8E0D8] mb-8">
            ...
            <button
              ...
              className={cn(
                ...
                activeTab === tab
                  ...
                  : "border-transparent text-[#6B6056] hover:text-[#1A1A1A]"
              )}
            >
        ...
        {activeTab === "products" ? (
          activeProducts.length > 0 ? (
            ...
            <div className="py-16 text-center">
              <p className="text-[#6B6056]">No products listed yet.</p>
            </div>
          )
        ) : (
          <div className="max-w-2xl pb-12 space-y-6">
            {brand.brandStory && (
              <div>
                ...
                <p className="text-[#6B6056] leading-relaxed">{brand.brandStory}</p>
              </div>
            )}
            {brand.description && (
              <div>
                <h2 className="font-heading text-xl font-semibold text-[#1A1A1A] mb-3">About</h2>
                <p className="text-[#6B6056] leading-relaxed">{brand.description}</p>
              </div>
            )}
            <div className="flex gap-4">
              {brand.instagramHandle && (
                <a
                  ...
                  className="flex items-center gap-1.5 text-sm text-[#C8956C] hover:text-[#B07D57] transition-colors"
                >
...
function StatItem({ icon, value, label }: { icon: React.ReactNode; value: string | number; label: string }) {
  return (
    <div className="flex items-center gap-2 text-sm text-[#6B6056]">
      <span className="text-[#C8956C]">{icon}</span>
...
function BrandSkeleton() {
  return (
    <div>
      <div className="h-64 bg-[#F5EDE6] animate-pulse" />
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6 space-y-4">
        <div className="flex items-center gap-4">
          <div className="h-20 w-20 rounded-2xl bg-[#F5EDE6] animate-pulse" />
          <div className="space-y-2">
            <div className="h-7 w-48 bg-[#F5EDE6] rounded animate-pulse" />
            <div className="h-4 w-32 bg-[#F5EDE6] rounded animate-pulse" />
          </div>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6">
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="aspect-square rounded-xl bg-[#F5EDE6] animate-pulse" />
          ))}
        </div>
      </div>
    </div>
