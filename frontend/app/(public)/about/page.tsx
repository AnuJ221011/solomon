  ...
  return (
    <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 py-16 space-y-20">
      ...
      <div className="text-center max-w-2xl mx-auto">
        <p className="text-sm font-medium text-[#C8956C] uppercase tracking-widest mb-3">How it works</p>
        <h1 className="font-heading text-5xl font-bold text-[#1A1A1A] leading-tight">
          India's B2B wholesale marketplace for the world.
        </h1>
        <p className="mt-4 text-lg text-[#6B6056] leading-relaxed">
      ...
      <div>
        ...
        <p className="text-[#6B6056] mb-8">Independent boutiques, gift shops, subscription boxes, and online stores.</p>
        <div className="grid sm:grid-cols-2 gap-6">
          {BUYER_STEPS.map((s) => (
            <div key={s.n} className="flex gap-4 p-5 bg-white rounded-xl border border-[#E8E0D8] shadow-warm hover:border-[#C8956C] transition-all">
              <span className="font-heading text-2xl font-bold text-[#E8E0D8] shrink-0">{s.n}</span>
              <div>
                <h3 className="font-semibold text-[#1A1A1A] mb-1">{s.title}</h3>
                <p className="text-sm text-[#6B6056] leading-relaxed">{s.body}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="pt-8 border-t border-[#E8E0D8]">
        ...
        <p className="text-[#6B6056] mb-8">Reach international retailers without a dedicated sales team.</p>
        <div className="grid sm:grid-cols-2 gap-6">
          {BRAND_STEPS.map((s) => (
            <div key={s.n} className="flex gap-4 p-5 bg-white rounded-xl border border-[#E8E0D8] shadow-warm hover:border-[#C8956C] transition-all">
              <span className="font-heading text-2xl font-bold text-[#E8E0D8] shrink-0">{s.n}</span>
      ...
      <div className="bg-[#1A1A1A] rounded-2xl p-8 text-white">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="text-[#6B6056] border-b border-[#2D2D2D]">
            ...
            <tbody className="divide-y divide-[#2D2D2D]">
              {[
                ...
                <tr key={String(scenario)}>
                  <td className="py-3 text-[#E8E0D8]">{scenario}</td>
                  <td className={`py-3 text-right font-semibold ${highlight ? "text-[#C8956C]" : "text-white"}`}>{rate}</td>
                </tr>
              ))}
            </tbody>
          </table>
